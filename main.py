...
def log_activity(db: Session, user_id: int, action_type: str, details: str):
    activity = UserActivity(
        user_id=user_id, action_type=action_type, action_details=details
    )
    db.add(activity)
    db.commit()
...
def register_user(user: UserCreate, db: Session = Depends(get_db)):
    ...
    if existing_user:
        logging.error(
            f"Registration failed: Username {user.username} or email {user.email} already exists"
        )
        raise HTTPException(
...
def admin_create_user(
    ...
    if existing_user:
        logging.error(
            f"User creation failed: Username {user.username} or email {user.email} already exists"
        )
        raise HTTPException(
            status_code=400, detail="Username or email already registered"
        )
...
def update_user(
    ...
    if existing_user:
        logging.error(
            f"User update failed: Username {user.username} or email {user.email} already exists"
        )
        raise HTTPException(
            status_code=400, detail="Username or email already registered"
        )
...
def create_vehicle(
    ...
    if existing_vehicle:
        logging.error(
            f"Vehicle creation failed: Registration number {vehicle.registration_number} already exists"
        )
        raise HTTPException(
    ...
    log_activity(
        ...
        current_user.id,
        "add_vehicle",
        f"Added vehicle {vehicle.registration_number}",
    )
    return new_vehicle
...
def update_vehicle(
    ...
    if existing_vehicle:
        logging.error(
            f"Vehicle update failed: Registration number {vehicle.registration_number} already exists"
        )
        raise HTTPException(
            status_code=400, detail="Vehicle registration number already exists"
        )
    ...
    log_activity(
        ...
        current_user.id,
        "update_vehicle",
        f"Updated vehicle {vehicle.registration_number}",
    )
    return db_vehicle
...
def delete_vehicle(
    ...
    log_activity(
        ...
        current_user.id,
        "delete_vehicle",
        f"Deleted vehicle {db_vehicle.registration_number}",
    )
...
def create_driver(
    ...
    if existing_driver:
        logging.error(
            f"Driver creation failed: License number {driver.license_number} already exists"
        )
        raise HTTPException(
            status_code=400, detail="Driver license number already exists"
        )
...
def update_driver(
    ...
    if existing_driver:
        logging.error(
            f"Driver update failed: License number {driver.license_number} already exists"
        )
        raise HTTPException(
            status_code=400, detail="Driver license number already exists"
        )
...
def delete_driver(
    ...
    db.commit()
    log_activity(
        db, current_user.id, "delete_driver", f"Deleted driver {db_driver.name}"
    )
...
async def upload_driver_document(
    ...
    file_name = (
        f"driver_doc_{driver_id}_{datetime.now().strftime('%Y%m%d%H%M%S')}.{file_ext}"
    )
    ...
    log_activity(
        ...
        current_user.id,
        "upload_driver_doc",
        f"Uploaded {doc_type} for driver {driver_id}",
    )
    return document
...
async def upload_vehicle_document(
    ...
    if not vehicle:
        logging.error(
            f"Vehicle document upload failed: Vehicle ID {vehicle_id} not found"
        )
        raise HTTPException(status_code=404, detail="Vehicle not found")
    file_ext = file.filename.split(".")[-1]
    file_name = (
        f"vehicle_doc_{vehicle_id}_{datetime.now().strftime('%Y%m%d%H%M%S')}.{file_ext}"
    )
    save_path = Path("documents") / file_name
    save_path.parent.mkdir(exist_ok=True)
    with open(save_path, "wb") as f:
    ...
    log_activity(
        ...
        current_user.id,
        "upload_vehicle_doc",
        f"Uploaded {doc_type} for vehicle {vehicle_id}",
    )
    return document
...
def list_vehicle_documents(
    ...
    if not vehicle:
        logging.error(
            f"Vehicle document fetch failed: Vehicle ID {vehicle_id} not found"
        )
        raise HTTPException(status_code=404, detail="Vehicle not found")
    return (
        db.query(VehicleDocument).filter(VehicleDocument.vehicle_id == vehicle_id).all()
    )
...
async def create_cost(
    ...
    db.refresh(new_cost)
    log_activity(
        db, current_user.id, "add_cost", f"Added {cost.category} cost of {cost.amount}"
    )
    return new_cost
...
def create_maintenance_record(
    ...
    if not vehicle:
        logging.error(
            f"Maintenance record creation failed: Vehicle ID {record.vehicle_id} not found"
        )
        raise HTTPException(status_code=404, detail="Vehicle not found")
    ...
    log_activity(
        ...
        f"Scheduled {record.maintenance_type} for vehicle {record.vehicle_id}",
    )
    return new_record
...
def optimize_route(
    ...
    carbon_reduction = (
        fuel_saved * 2.68 * vehicle.fuel_level if vehicle.fuel_level else 0
    )
    log_activity(
        db,
        current_user.id,
        "optimize_route",
        f"Optimized route for vehicle {vehicle_id}",
    )
    return {
        "vehicle_id": vehicle_id,
...
def get_simulated_updates(
    ...
    for vehicle in vehicles:
        ...
        vehicle.maintenance_score = max(
            0, vehicle.maintenance_score - random.uniform(0, 0.2)
        )
    ...
    log_activity(
        ...
        current_user.id,
        "simulate_updates",
        "Fetched simulated vehicle and driver updates",
    )
    return {
        ...
        while True:
            ...
            if vehicle:
                ...
                await websocket.send_json(
                    {"status": "updated", "vehicle_id": vehicle_id}
                )
...
async def restore_backup(
    ...
    try:
        ...
        log_activity(
            db, admin_user.id, "restore", f"Restored database from: {backup_path}"
        )
